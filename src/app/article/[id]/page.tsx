...
export default async function ArticlePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isPaywallText = (text?: string) => Boolean(text && /only available in paid plans|subscribe to read|sign in to read|unlock this article|premium content|become a member|access to this article/i.test(text));
  const suppliedText = [article.content, article.description].find((text) => text && !isPaywallText(text)) || "This story is available as a summary from the news provider.";
